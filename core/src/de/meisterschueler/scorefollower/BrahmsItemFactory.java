...
public class BrahmsItemFactory implements MatchingItemFactory {
	...
	private Song getNo(int no) {
		...
		case 0: {
			...
			rightHand = guidoService.gmnToScores(rightPattern);
			leftHand = guidoService.gmnToScores(leftPattern);
			break;
		}
...
//		}
		...
		case 7: {
			...
			rightHand = guidoService.gmnToScores(guidoService.oneOctaveUp(pattern1), rightFingers1);
			rightHand.addAll(guidoService.gmnToScores(guidoService.oneOctaveUp(pattern2right), rightFingers2));
		...
		song.setLeftScores(leftHand);
		song.setRightScores(rightHand);
